...
class OutreachRecord(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    # Pipeline status
    status: Mapped[PipelineStage] = mapped_column(SAEnum(PipelineStage), default=PipelineStage.SCORED)
    ...
    call_disposition: Mapped[Optional[CallDisposition]] = mapped_column(SAEnum(CallDisposition), nullable=True)
    ...
    meeting_type: Mapped[Optional[MeetingType]] = mapped_column(SAEnum(MeetingType), nullable=True)
