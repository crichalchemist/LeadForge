...
class TestRankedLeads:
    ...
    async def test_ranked_with_scores(
        self, client, auth_headers, sample_business_with_score
    ):
    ...
    async def test_filter_by_price_tier(
        self, client, auth_headers, sample_business_with_score
    ):
...
class TestScoreHistory:
    ...
    async def test_no_scores(self, client, auth_headers, sample_business):
        resp = await client.get(
            f"/leads/{sample_business.id}/score", headers=auth_headers
        )
